...
const CADENCE_SCRIPTS = {
  ...
            if market.status == FlowWagerV2.MarketStatus.PendingResolution {
                // NOTE: FlowWagerV2.getResolutionEvidence does not exist.
                // You must add a public getter to the contract to see evidence.

                // Total volume is stored in totalPool
                let totalVolume = market.totalPool
  ...
                // Get participant count from the public function
                let participantCount = UInt64(FlowWagerV2.getMarketParticipants(marketId: market.id).length)
