...
import { Activity, TrendingUp, Users, DollarSign, Clock, Target } from 'lucide-react';
...
export function MarketActivity({ marketId, marketTitle, optionA, optionB }: MarketActivityProps) {
  ...
  const fetchMarketActivities = async () => {
    try {
      ...
      console.log('✅ Activities fetched:', activitiesData?.length || 0, 'records');
      ...
      if (activitiesData && activitiesData.length > 0) {
        const uniqueBettors = new Set(activitiesData.map(a => a.user_address)).size;
        const betActivities = activitiesData.filter(a => a.activity_type === 'place_bet');
        ...
        const optionAVolume = betActivities
          .filter(a => a.details?.option === 0 || a.details?.outcome === optionA)
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-[#0A0C14]/50 rounded-lg p-3 border border-gray-800/30">
            ...
            <p className="text-lg font-bold text-white">{stats.totalVolume.toFixed(2)} FLOW</p>
          </div>
          ...
          <div className="bg-[#0A0C14]/50 rounded-lg p-3 border border-gray-800/30">
            ...
            <p className="text-lg font-bold text-white">{stats.optionAVolume.toFixed(2)} FLOW</p>
          </div>
          ...
          <div className="bg-[#0A0C14]/50 rounded-lg p-3 border border-gray-800/30">
            ...
            <p className="text-lg font-bold text-white">{stats.optionBVolume.toFixed(2)} FLOW</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-gray-800/50">
            <TabsTrigger value="all">All Bets ({activities.length})</TabsTrigger>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-400">
                        <span>Bet {activity.details?.betAmount?.toFixed(2)} FLOW</span>
                        <span>on</span>
