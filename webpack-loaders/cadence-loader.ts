  ...
  try {
    ...
    const contractAddresses: NetworkConfig = {
      mainnet: {
        ...
        FlowWager:
          process.env.NEXT_PUBLIC_FLOWWAGER_CONTRACT || "0x6c1b12e35dca8863",
      },
      testnet: {
    ...
    const addresses: ContractAddresses =
      contractAddresses[network] || contractAddresses.testnet;

    let processedSource: string = source;

    Object.keys(addresses).forEach((contractName: string) => {
      const importRegex = new RegExp(
        `import\\s+${contractName}\\s+from\\s+[\\"\\'](${contractName}|.*?)[\\"\\']/`,
        "g"
      );
      processedSource = processedSource.replace(
        importRegex,
        `import ${contractName} from ${addresses[contractName]}`
      );

      const simpleImportRegex = new RegExp(
        `import\\s+${contractName}\\s+from\\s+[\\"\\']${contractName}[\\"\\']`,
        "g"
      );
      processedSource = processedSource.replace(
    ...
    processedSource = processedSource.replace(
      /FUNGIBLE_TOKEN_ADDRESS/g,
      addresses.FungibleToken
    );
    processedSource = processedSource.replace(
      /FLOW_TOKEN_ADDRESS/g,
      addresses.FlowToken
    );
